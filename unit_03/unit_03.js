...
function t3() {
  let data = document.querySelector(".ch-3");
  if (data.checked) {
    return data.value;
  } else {
    return false;
...
function t4() {
  let data = document.querySelectorAll(".r-4");
  for (let elem of data) {
    if (elem.checked) {
      return (document.querySelector(".out-4").innerHTML = elem.value);
    }
  }
}
...
function t5() {
  ...
  let sign = document.querySelector(".s52").value;
...
// Task 21
// Напишите функцию, которая в зависимости от параметра even или odd возвращает случайное целое четное (even) или нечетное (odd) число от 10 до 20.
...
// Не нашел решения. До конца не решена.
...
document.querySelector(".b-25").addEventListener("click", function () {
  document.querySelector(".out-25").textContent = t25([4, 5, 6, 7]);
});
...
function t26() {
  ...
  if (str != "") {
    out.innerHTML = str.toLowerCase().trim();
  } else return;
}
