...
function t1(elem) {
  ...
  function r1(element) {
    ...
    out += element.tagName + ' '
    Array.from(arr).forEach(item => r1(item))
  }
  return(out)
}
...
function t2(elem) {
  ...
  function r2(element) {
   ...
   if(!isNaN(element.textContent)){
    ...
    Array.from(arr).forEach(item => r2(item))
  }
  return s
}
...
function t3(elem) {
  ...
  r3(elem)
   ...
   function r3(element) {
   let arr = element.children
    if(arr.length === 0){     // arr.length - подсчитывает количество вложенных тегов.
      //Пояснение для меня: если нет в arr вложенных элементов тогда применяем textContent
      out += element.textContent
   }
   Array.from(arr).forEach(item => r3(item))
 }
 return out
}
...
function t4(elem) {
  ...
  function r4(element) {
    let arr = element.children
    if(arr.length === 0){   // arr.length - подсчитывает количество вложенных тегов.
       //Пояснение для меня: если нет в arr вложенных элементов тогда добавить style.background = 'orange'
       element.style.background = 'orange'
      }
      Array.from(arr).forEach(item => r4(item))
  }
}
...
function t5(elem) {
  ...
  function r5(element) {
    let arr = element.children
    if(arr.length === 0){  //замена тегов на тег mark
      element.outerHTML = `<mark> ${element.textContent} </mark>`
      }
      Array.from(arr).forEach(item => r5(item))
}
}
