...
function t1(min, max) {
  let num1 = document.querySelector(".out-1");
  num1.innerHTML = Math.floor(min + Math.random() * (max + 1 - min));
}
...
function t2(min, max, block) {
  block.innerHTML = Math.floor(min + Math.random() * (max + 1 - min));
}
...
function t3(min = 0, max = 100) {
  ...
  out3.innerHTML = Math.floor(min + Math.random() * (max + 1 - min));
}
